...
# %%
"""BO Active Sampling with Estimated Field Planning (full-grid variant, no subgrid ROI)."""
...
_parser.add_argument(
    ...
    "--name",
    default="default",
    help="Subdirectory under plots/est_bo_active_full/ for this run's outputs.",
)
_args, _ = _parser.parse_known_args()
RUN_NAME = Path(_args.name).name or "default"
PLOT_RUN_DIR = SCRIPT_DIR / "plots" / "est_bo_active_full" / RUN_NAME
...
# %%
# 2. Setup Full GP Test Grid for predictions (also the full candidate set for BO)
# ===============================================================================
...
# ==================================================
def h_rmse(H_est, H_true, max_cost=oob_penalty_max):
...
def run_trajectory_loops(
    ...
    for step_i in range(m_random):
        ...
        # Regret over all observations on the full grid
    ...
    rmse_history = []
    # (n_samples, h_rmse_full, vi_iters after each GP update / re-plan)
    h_rmse_history = []
    loop_idx = 1
    targets_attempted = 0
    targets_reached = 0
...
axes[0, 2].set_title(f"H (true field) from center ({initial_pos.i},{initial_pos.j})")
...
# ===========================================================================
fig_h, axes_h = plt.subplots(1, 2, figsize=(11, 4.8))
...
axes_h[1].set_ylabel("Value-iteration sweeps")
axes_h[1].set_title("AP-SSP iterations (after each GP fit)")
