...
class AbstractArena(ABC):
    ...
    The arena contains the following components:
    ...
    @abstractmethod
    def reset(self, rng_key: jnp.ndarray, **kwargs) -> np.ndarray:
        """Reset arena to initial state.

        Args:
            rng_key: JAX PRNG key for reproducible initialization.
            **kwargs: Optional soft reset arguments. Callers passing only rng_key get default full reset.
